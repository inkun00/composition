...
function isHostedAppHostname(hostname) {
  return hostname === "maeum-melody.vercel.app" ||
    (hostname.startsWith("maeum-melody-") && hostname.endsWith(".vercel.app"));
}

function isTrustedOrigin(request, allowHostedOrigin) {
  const origin = request.headers.origin;
  if (!origin) return true;
  try {
    const parsed = new URL(origin);
    return isLoopbackHostname(parsed.hostname) ||
      (allowHostedOrigin && parsed.protocol === "https:" && isHostedAppHostname(parsed.hostname));
  } catch {
    return false;
  }
}

function applyCorsHeaders(request, response, allowHostedOrigin) {
  const origin = request.headers.origin;
  if (!origin || !isTrustedOrigin(request, allowHostedOrigin)) return;
  response.setHeader("Access-Control-Allow-Origin", origin);
  response.setHeader("Access-Control-Allow-Methods", "GET, POST, OPTIONS");
  response.setHeader("Access-Control-Allow-Headers", "Accept, Content-Type, X-Score-Filename");
...
export async function homrMiddleware(request, response, next, options = {}) {
  ...
  const allowHostedOrigin = Boolean(options.allowHostedOrigin);
  if (!isTrustedOrigin(request, allowHostedOrigin)) {
    sendError(response, 403, "UNTRUSTED_ORIGIN", "이 앱 화면에서만 악보를 읽을 수 있어요.");
    return;
  }
  applyCorsHeaders(request, response, allowHostedOrigin);
